...
#' @keywords evaluation
#' @export GATE
GATE <- function(D, tau, Y, ngates = 5) {
  ...
  for (i in 1:ngates) {
    ...
    gates[i] = ngates * (1/n1*sum(D*That*Y)+1/n0*sum(Y*(1-D)*(1-That))-1/n0*sum(Y * (1-D)))
    Sfp1 = var((That*Y)[D==1])
    Sfp0 = var((That*Y)[D==0])
    if (length(Y[D==1 & That==1])>0 & length(Y[D==0 & That==1])>0) {
      kf1 = mean(Y[D==1 & That==1])-mean(Y[D==0 & That==1])
